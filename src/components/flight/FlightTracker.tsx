...
const STATUS_COL: Record<string, string> = {
  LIVE: 'var(--g)',
  STALE: 'rgba(255,215,0,.9)',
  OFFLINE: 'rgba(255,140,0,.9)',
  SIMULATED: 'var(--g3)',
}
...
export function FlightTracker() {
  ...
  const { flights, status, source } = useOpenSky()
  ...
  useEffect(() => {
    ...
    const map = L.map(mapRef.current, {
      center: [27, 50], zoom: 6, zoomControl: false, attributionControl: false, minZoom: 4, maxZoom: 10,
    })
    mapInstance.current = map
    L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', {
      subdomains: 'abcd', maxZoom: 19, className: 'ft-tiles',
    }).addTo(map)
  ...
  useEffect(() => {
    ...
    flights.forEach(f => {
      ...
      L.polyline(trail, { color: col.replace(/[\d.]+\)$/, '.2)'), weight: 1, dashArray: '3 4' }).addTo(map)
      const icon = L.divIcon({ html: chevronSvg(f.heading, col), className: '', iconSize: [20, 20], iconAnchor: [10, 10] })
      const marker = L.marker([f.lat, f.lng], { icon })
        .addTo(map)
        .bindTooltip(buildTooltip(f), { direction: 'right', offset: [12, 0], className: 'ft-tooltip', permanent: false })
  ...
  return (
    <div className="ft-section sev-nominal">
      <div className="ft-header jp-panel-header">
        <div className="HDR-DOT jp-status-dot" style={{ background: STATUS_COL[status] || 'var(--g3)' }} />
        <span className="ft-title">AIRSPACE TRACKER</span>
        <span className="ft-count">{counts.total} TRACKS · {status}{source === 'demo' ? ' (SIM)' : ''}</span>
